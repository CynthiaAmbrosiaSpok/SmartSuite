...
public class SearchActiveMessagesTest extends TestBase {
	
	//Define Variable(s)
	SoftAssert checkpoint;
	
	//Constructor
	public SearchActiveMessagesTest() {
		super();
	}
	...
	public void beforeClass() {
		...
		excelMethods.setSheetName("Search Active Messages");
	}
	
	@Test(dataProvider="inputs", dataProviderClass=ExcelMethods.class)
	public void testcases_personalProfile_Inbox_SearchActiveMessagesTest(String active, String reportTitle, String recipient, String pageMessage, String searchSender, String searchMessage, String searchDate, String expectedSender, String expectedMessage, String expectedDate, String dataRow) {
...
//			DateTimeFormatter expectedArchiveDateFormatter = DateTimeFormatter.ofPattern("dd-MMM-yyyy HH:mm a");
		...
		if (active.equalsIgnoreCase("y") || active.equalsIgnoreCase("yes")) {
			...
			performSetup(reportTitle, "testcases_PersonalProfile_Inbox_SearchActiveMessagesTest()");
			...
			personalProfileInboxPage.searchActiveMessages(searchSender, searchMessage, searchDate, searchDate);
			...
			if (!searchSender.equals("")) {
				...
				personalProfileInboxPage.verifyActiveMessageSearchResultSenderLabel(checkpoint, expectedSender);
			}
			...
			if (!searchMessage.equals("")) {
				...
				personalProfileInboxPage.verifyActiveMessageSearchResultMessageLabel(checkpoint, expectedMessage);
			}
			
			//If 'Date Entered' search criteria was used, verify the 'Date Entered' info from search result(s) matches expectations
			if (!searchDate.equals("")) {
				//Checking if the search result's 'Date Entered' info matches expectation(s)
				personalProfileInboxPage.verifyActiveMessageSearchResultEnteredDateLabel(checkpoint, expectedDate);
			}
